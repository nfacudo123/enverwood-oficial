...
const UtilAdmin: React.FC = () => {
  ...
  const fetchUtilidades = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error al obtener utilidades');
      }
    } catch (error) {
  ...
  const filterUtilidades = () => {
    const filtered = utilidades.filter(utilidad =>
      utilidad.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      utilidad.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, 'utilidades.xlsx');
  };
  ...
  if (loading) {
    return (
      <OrganizationLayout title="Utilidades">
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-primary"></div>
            <p className="mt-4 text-muted-foreground">Cargando utilidades...</p>
          </div>
        </div>
      </OrganizationLayout>
    );
  ...
  return (
    <OrganizationLayout title="Administración de Utilidades">
          ...
          <CardContent>
            <div className="mb-6">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar por nombre, email o valor..."
                  value={searchTerm}
                  ...
                  {currentItems.length > 0 ? (
                    currentItems.map((utilidad) => (
                      <TableRow key={utilidad.id}>
                        ...
                        <TableCell className="font-medium">{utilidad.name}</TableCell>
                        <TableCell>{utilidad.email}</TableCell>
